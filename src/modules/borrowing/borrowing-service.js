...
class BorrowingService {
  ...
  async approveBorrowing(studentId, bookId) {
    ...
    try {
      ...
      if (!student) {
        throw new Error(
          `Can not borrow the book due student[${studentId}] does not exist!`
        );
      }
      if (!book) {
        throw new Error(
          `Can not borrow the book due book[${bookId}] does not exist!`
        );
      }
      if (borrow) {
        throw new Error(
          `Can not borrow the book due the book[${bookId}] is already in use!`
        );
      }
      if (!student.borrowing_limit) {
        throw new Error(
          `The student[${studentId}] is not able to borrow the book due the borrowing limit!`
        );
      }
      ...
      await Promise.all([
        this.repository.create(
          new BorrowingModel({
            studentId,
            bookId,
            returnTermDays: book.return_term_days,
          }),
          transaction
        ),
  ...
  async acceptReturn(studentId, bookId) {
    const [borrowing] = await this.repository.find({
    ...
    try {
      ...
      if (!student) {
        throw new Error(
          `Can not return the book due student[${studentId}] does not exist!`
        );
      }
