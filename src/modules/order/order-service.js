...
class OrderService {
  ...
  async create(bookIds = [], studentId) {
    ...
    if (!student) {
      throw new Error(
        `Can not create the order due student[${studentId}] does not exist!`
      );
    }
    if (bookIds.length > student.borrowing_limit) {
      throw new Error(
        `You are out of borrowing limit! You can borrow only ${student.borrowing_limit} books.`
      );
    }
    ...
    const promises = bookIds.map((bookId) => {
      return this.orderBookRepository.create(
        new OrderBookModel({ bookId, orderId: order.id })
      );
    });
